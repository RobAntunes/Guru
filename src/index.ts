...
const InferPurposeZod = z.object({
  symbolId: z.string().optional().describe('Specific symbol to analyze purpose for'),
  codeBlock: z.string().optional().describe('Code block to analyze directly'),
  context: z.string().optional().describe('Additional context for inference')
});
...
const InferPurposeSchema = {
  type: "object" as const,
  properties: {
    symbolId: { type: "string", description: "Specific symbol to analyze purpose for" },
    codeBlock: { type: "string", description: "Code block to analyze directly" },
    context: { type: "string", description: "Additional context for inference" }
  }
};
...
const tools: Tool[] = [
  ...
  {
    name: 'infer_purpose',
    description: 'Infer the purpose and goals of code through structural analysis',
    inputSchema: InferPurposeSchema,
  },
  {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'infer_purpose': {
        const params = InferPurposeZod.parse(args);
        const result = await guru.inferPurpose(params);
        return {
          content: [
            {
              type: 'text',
