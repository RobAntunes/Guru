...
export class EntryPointDetector {
  ...
  async detectEntryPoints(
    ...
    const primaryEntryPoint =
      entryPoints.find((ep) => ep.priority === "primary") || entryPoints[0];
    ...
    const analysis: EntryPointAnalysis = {
      ...
      analysisMetadata: {
        ...
        highConfidenceCount: entryPoints.filter((ep) => ep.confidence > 0.8)
          .length,
        mediumConfidenceCount: entryPoints.filter(
          (ep) => ep.confidence > 0.6 && ep.confidence <= 0.8,
        ).length,
        lowConfidenceCount: entryPoints.filter((ep) => ep.confidence <= 0.6)
          .length,
      },
    };
   ...
   */
  private async findPackageJsonEntryPoints(
    rootPath: string,
  ): Promise<EntryPointCandidate[]> {
    const candidates: EntryPointCandidate[] = [];
  ...
  private async scoreCandidates(
    ...
    for (const candidate of candidates) {
      ...
      if (relativePath.includes("src/") && relativePath.includes("index."))
        bonusScore += 0.05;
  ...
  private async buildEntryPoints(
    ...
    for (const candidate of candidates) {
      ...
      const entryPointType = this.determineEntryPointType(
        candidate,
        executionContext,
      );
  ...
  private async analyzeExecutionContext(
    ...
    try {
      ...
      if (content.includes("window.onload"))
        context.triggers.push("window_load");
  ...
  private determineEntryPointType(
    ...
    if (
      candidate.name.toLowerCase().includes("cli") ||
      context.triggers.includes("cli_args")
    )
      return "cli";
    if (
      candidate.name.toLowerCase().includes("server") ||
      context.triggers.includes("server_start")
    )
      return "server";
    if (
      candidate.name.toLowerCase().includes("test") ||
      context.environment === "test"
    )
      return "test";
