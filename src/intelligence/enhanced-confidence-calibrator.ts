...
export interface ConfidenceFactor {
  type: 'evidence_quality' | 'evidence_quantity' | 'context_clarity' | 'pattern_strength' | 'naming_clarity';
...
export class SmartConfidenceCalibrator {
  ...
  calibrateConfidence(
    ...
    if (symbol.name && symbol.name.length === 1 && !this.isAcceptableSingleChar(symbol.name)) {
      const penalty = -0.25;
      penalties += penalty;
      factors.push({
        type: 'naming_clarity',
        impact: penalty,
        reasoning: 'Single character variable name provides minimal semantic information'
      });
    }
    ...
    const totalAdjustment = evidenceBoost + contextBoost + qualityBoost + penalties;
    const calibrated = Math.max(0.05, Math.min(0.98, baseConfidence + totalAdjustment));
  ...
  enhancePurposeConfidence(
    ...
    if (similarFunctions.length >= 2) {
      ...
      boosters.push(`Cross-validated with ${similarFunctions.length} similar functions`);
    }
    
    // 5. Documentation Alignment (+0.15)
    if (symbol.metadata.docstring && this.docAlignsPurpose(symbol.metadata.docstring, basePurpose)) {
      confidenceBoost += 0.15;
   ...
   */
  private assessPatternStrength(symbol: SymbolNode, node: Parser.SyntaxNode, evidence: Evidence[]): number {
    let strength = 0;
  ...
  private estimateComplexity(node: Parser.SyntaxNode): number {
    ...
    // Decision points
    complexity += (text.match(/\b(if|else|switch|case|while|for|do)\b/g) || []).length * 2;
  ...
  private hasStrongSemanticSignal(name: string): boolean {
    const strongPatterns = [
      /validate/i, /transform/i, /process/i, /handle/i, /create/i, /generate/i,
      /parse/i, /format/i, /calculate/i, /compute/i, /analyze/i, /extract/i,
      /filter/i, /mapper?/i, /reduce/i, /sort/i, /find/i, /search/i
    ];
   ...
   */
  private hasConsistentParameterTypes(symbol: SymbolNode, node: Parser.SyntaxNode): boolean {
    ...
    for (const param of params) {
      const paramUsage = (bodyText.match(new RegExp(`\\b${param}\\b`, 'g')) || []).length;
   ...
   */
  private hasTypicalReturnPattern(symbol: SymbolNode, node: Parser.SyntaxNode, purpose: string): boolean {
    const bodyText = node.text;
    
    // Validation functions should return boolean
    if (purpose.includes('valid') && /return\s+(true|false|[^;]+===|[^;]+!==)/.test(bodyText)) {
      return true;
    }
    
    // Transform functions should return modified data
    if (purpose.includes('transform') && /return\s+\w+\.\w+|return\s+\{/.test(bodyText)) {
      return true;
    }
    
    // Filter functions should return boolean
    if (purpose.includes('filter') && /return\s+(true|false|[^;]+[<>=]=?)/.test(bodyText)) {
      return true;
    }
   ...
   */
  private findSimilarFunctions(symbol: SymbolNode, purpose: string): SymbolNode[] {
  ...
  private docAlignsPurpose(docstring: string, purpose: string): boolean {
    ...
    const purposeWords = purposeLower.split(/\s+/).filter(word => word.length > 3);
   ...
   */
  private assessContextStrength(symbol: SymbolNode, node: Parser.SyntaxNode): number {
    let strength = 0;
  ...
  private calculatePurposeSpecificity(purpose: string): number {
    ...
    const genericTerms = ['transforms', 'processes', 'handles', 'performs', 'operation'];
    if (genericTerms.some(term => purpose.toLowerCase().includes(term))) {
      return 0.3;
    }
    
    // Specific purposes get high specificity
    const specificTerms = ['validates', 'extracts', 'filters', 'converts', 'calculates', 'formats'];
