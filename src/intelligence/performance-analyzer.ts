...
export class PerformanceAnalyzer {
  ...
  async analyzePerformance(symbolGraph: SymbolGraph): Promise<any> {
    ...
    console.log(`⚡ Performance analysis complete! Found ${issues.length} issues, ${optimizations.length} optimization opportunities`);
    return analysis;
  }
   ...
   */
  private async detectPerformanceIssues(symbolGraph: SymbolGraph): Promise<any[]> {
    const issues: any[] = [];
    ...
    for (const [symbolId, symbol] of symbolGraph.symbols) {
      ...
      // Detect O(n²) patterns
      if (/nested|double.*loop|find.*duplicate|compare.*all|sort.*bubble/i.test(name)) {
        issues.push({
      ...
      if (/build.*string|concat.*string|string.*loop/i.test(name)) {
        issues.push({
          ...
          impact: 'String concatenation in loops creates O(n²) memory allocation',
          confidence: 0.6
        });
      }
      
      // Detect blocking operations
      if (/sync|blocking|wait|heavy.*computation/i.test(name) && !/async|promise|await/i.test(name)) {
        issues.push({
   ...
   */
  private async findOptimizationOpportunities(symbolGraph: SymbolGraph): Promise<any[]> {
    ...
    for (const [symbolId, symbol] of symbolGraph.symbols) {
      ...
      if (/get|fetch|load|retrieve|calculate/i.test(name) && !/cache/i.test(name)) {
        opportunities.push({
          type: 'caching',
          symbolId,
          symbolName: symbol.name,
          description: `Add caching to ${symbol.name}`,
          expectedImprovement: 'Reduce redundant computations and data fetching',
          effort: 'medium',
          confidence: 0.6
        });
      }
      ...
      if (/recursive|calculate|compute|expensive/i.test(name) && !/memo/i.test(name)) {
        opportunities.push({
          type: 'memoization',
          symbolId,
          symbolName: symbol.name,
          description: `Add memoization to ${symbol.name}`,
          expectedImprovement: 'Cache function results to avoid redundant calculations',
          effort: 'low',
          confidence: 0.7
        });
      }
   ...
   */
  private generateRecommendations(issues: any[], optimizations: any[]): string[] {
    const recommendations: string[] = [];
    
    const criticalIssues = issues.filter(i => i.severity === 'high');
    if (criticalIssues.length > 0) {
      recommendations.push(`Address ${criticalIssues.length} critical performance issues first`);
    }
    
    const easyOptimizations = optimizations.filter(o => o.effort === 'low');
    if (easyOptimizations.length > 0) {
      recommendations.push(`Implement ${easyOptimizations.length} low-effort optimizations for quick wins`);
    }
    
    if (recommendations.length === 0) {
      recommendations.push('Performance looks good - monitor for regressions as codebase grows');
    }
