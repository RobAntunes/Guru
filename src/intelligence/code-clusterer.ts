...
export class CodeClusterer {
  ...
  async clusterSymbols(
    ...
    // Default options
    const {
      algorithms = ['hybrid_intelligent', 'connectivity_based', 'semantic_similarity'],
      minClusterSize = 2,
      maxClusterSize = 30,
    ...
    for (const algorithmName of algorithms) {
      const algorithm = CLUSTERING_ALGORITHMS.find(a => a.name === algorithmName);
      if (algorithm) {
        console.log(`🔍 Running ${algorithmName} clustering...`);
        const candidates = await this.runClusteringAlgorithm(symbolGraph, algorithm);
    ...
    const clusters = await this.buildFinalClusters(combinedClusters, symbolGraph);
    
    // Find unclustered symbols
    const clusteredSymbols = new Set(clusters.flatMap(c => c.symbols));
    const unclusteredSymbols = Array.from(symbolGraph.symbols.keys())
      .filter(id => !clusteredSymbols.has(id));
    ...
    const analysis: ClusteringAnalysis = {
      ...
      metadata: {
        ...
        averageClusterSize: clusters.length > 0 ? clusteredSymbols.size / clusters.length : 0,
    ...
    console.log(`✅ Clustering complete! Found ${clusters.length} clusters in ${processingTime}ms`);
    return analysis;
  }
  ...
  private async semanticSimilarityClustering(
    ...
    for (const [file, symbolIds] of fileGroups) {
      if (symbolIds.length >= algorithm.minClusterSize && symbolIds.length <= algorithm.maxClusterSize) {
        candidates.push({
          symbols: symbolIds,
          score: 0.8,
  ...
  private async combineClusteringResults(
    ...
    symbolGraph: SymbolGraph,
    options: { minClusterSize: number; maxClusterSize: number; confidenceThreshold: number }
  ): Promise<ClusterCandidate[]> {
    const allCandidates: ClusterCandidate[] = [];
    
    for (const [algorithm, candidates] of algorithmResults) {
      allCandidates.push(...candidates.filter(c => c.score >= options.confidenceThreshold));
    }
  ...
  // Helper methods
  private generateClusterName(symbolIds: string[], symbolGraph: SymbolGraph): string {
  ...
  private inferClusterPurpose(symbolIds: string[], symbolGraph: SymbolGraph): string {
    const size = symbolIds.length;
    return `Related functionality cluster (${size} symbols)`;
  }
  
  private calculateClusterConfidence(candidate: ClusterCandidate): ClusterConfidence {
    return {
  ...
  private calculateClusterMetrics(symbolIds: string[], symbolGraph: SymbolGraph): ClusterMetrics {
    return {
  ...
  private detectSemanticZone(symbolIds: string[], symbolGraph: SymbolGraph): SemanticZone {
    return {
      type: 'mixed',
      domain: 'general',
      layer: 'unknown',
  ...
  private calculateSizeDistribution(clusters: CodeCluster[]): Record<string, number> {
    ...
    for (const cluster of clusters) {
      const sizeRange = cluster.metrics.size <= 5 ? 'small' :
                       cluster.metrics.size <= 15 ? 'medium' : 'large';
  ...
  private calculateQualityMetrics(clusters: CodeCluster[], symbolGraph: SymbolGraph): any {
    ...
    const avgCohesion = clusters.reduce((sum, c) => sum + c.metrics.cohesion, 0) / clusters.length;
    const avgSeparation = clusters.reduce((sum, c) => sum + (1 - c.metrics.coupling), 0) / clusters.length;
