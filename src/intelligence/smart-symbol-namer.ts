...
import { SymbolNode, SmartSymbol, SymbolContext, ConfidenceScore, NamingStrategy } from '../types/index.js';
...
interface AISemanticContext {
  ...
  category: 'data_processing' | 'validation' | 'transformation' | 'event_handling' | 'utility' | 'business_logic';
...
export class SmartSymbolNamer {
  ...
  enhanceSymbol(
    ...
    const strategies = this.generateNamingStrategies(symbol, node, source, allSymbols);
    const bestStrategy = this.selectBestStrategy(strategies);
    
    const enhancedSymbol: SmartSymbol = {
      id: symbol.name === 'anonymous' ? this.generateSemanticId(symbol, node, source, bestStrategy.name) : symbol.id,
      inferredName: bestStrategy.name,
      originalName: symbol.name !== 'anonymous' ? symbol.name : undefined,
      confidence: this.calculateConfidenceScore(symbol, bestStrategy, node, source),
  ...
  private generateNamingStrategies(
    ...
    if (propertyContext) {
      strategies.push({
        ...
        confidence: 0.8,
        evidence: [`Object property: ${propertyContext.object}.${propertyContext.property}`],
    ...
    if (exportContext) {
      strategies.push({
        name: exportContext.type === 'default' ? 'default_export' : exportContext.name,
        confidence: 0.8,
    ...
    if (methodContext) {
      strategies.push({
        ...
        confidence: 0.8,
        evidence: [`Class method: ${methodContext.className}.${methodContext.methodName}`],
  ...
  private calculateIdentityConfidence(symbol: SymbolNode, strategy: NamingStrategy): number {
  ...
  private calculatePurposeConfidence(symbol: SymbolNode, node: Parser.SyntaxNode, source: string): number {
    ...
    if (symbol.metadata.parameters && symbol.metadata.parameters.length > 0) confidence += 0.1;
  ...
  private calculateImpactConfidence(symbol: SymbolNode, node: Parser.SyntaxNode): number {
  ...
  private identifyLimitations(symbol: SymbolNode, strategy: NamingStrategy, node: Parser.SyntaxNode): string[] {
   ...
   */
  private extractAssignmentContext(node: Parser.SyntaxNode, source: string): { name: string; pattern: string } | null {
   ...
   */
  private extractPropertyContext(node: Parser.SyntaxNode, source: string): { object: string; property: string } | null {
    let current = node.parent;
   ...
   */
  private extractCallbackContext(node: Parser.SyntaxNode, source: string): { method: string; parameter: string } | null {
    let current = node.parent;
   ...
   */
  private extractExportContext(node: Parser.SyntaxNode, source: string): { type: 'default' | 'named'; name: string } | null {
    let current = node.parent;
    
    while (current) {
      if (current.type === 'export_statement') {
        const defaultKeyword = current.children.find(child => child.text === 'default');
        if (defaultKeyword) {
   ...
   */
  private extractMethodContext(node: Parser.SyntaxNode, source: string): { className: string; methodName: string } | null {
    let current = node.parent;
  ...
  private hasSemanticNaming(name: string): boolean {
    // Avoid generic names
    const genericNames = ['func', 'function', 'method', 'callback', 'handler', 'temp', 'tmp', 'test'];
   ...
   * Analyze function body for semantic clues
   */
  private analyzeFunctionBody(node: Parser.SyntaxNode, source: string): {
    ...
    const hasErrorHandling = text.includes('try') || text.includes('catch') || text.includes('throw');
    if (hasErrorHandling) {
    ...
    if (text.includes('document.') || text.includes('element.') || text.includes('queryselector')) {
      patterns.push('dom-manipulation');
      semanticClues += 1;
    }
    
    // Check for event handling
    if (text.includes('addeventlistener') || text.includes('onclick') || text.includes('event')) {
      patterns.push('event-handling');
      semanticClues += 1;
    }
    
    // Check for data transformation
    if (text.includes('map') || text.includes('filter') || text.includes('reduce') || text.includes('transform')) {
    ...
    if (text.includes('console.') || text.includes('log') || text.includes('debug')) {
  ...
  private estimateComplexity(node: Parser.SyntaxNode, source: string): number {
    ...
    complexity += (text.match(/\b(if|else|switch|case|while|for|do)\b/g) || []).length;
   ...
   */
  private generateStableId(symbol: SymbolNode, node: Parser.SyntaxNode, source: string): string {
  ...
  private generateBehavioralNamingStrategy(
    ...
    const behavioralPattern = this.analyzeBehavioralPattern(node, source, semanticContext);
    
    if (behavioralPattern.type === 'unknown' || behavioralPattern.confidence < 0.4) {
      return null;
    }

    const semanticName = this.generateSemanticName(behavioralPattern, semanticContext);
   ...
   */
  private extractSemanticContext(node: Parser.SyntaxNode, source: string): SemanticContext {
  ...
  private analyzeBehavioralPattern(
    ...
    else if (context.methodName === 'reduce' && this.hasAccumulatorPattern(node, source)) {
      type = 'reducer';
    ...
    else if (patterns.get('dom_manipulation')! > 0.7 || bodyText.includes('event') || bodyText.includes('preventDefault')) {
      type = 'event_handler';
    ...
    // Validation patterns
    else if (patterns.get('data_validation')! > 0.7 && context.returnType === 'boolean') {
      type = 'validator';
    ...
    else if (context.propertyAccess.length === 1 && context.operations.length === 0) {
    ...
    else if (context.operations.includes('-') || context.operations.includes('<') || context.operations.includes('>')) {
    ...
    else if (context.returnType === 'boolean' || 
    ...
    else if (context.operations.includes('+') && this.hasAccumulatorPattern(node, source)) {
      type = 'accumulator';
      if (context.propertyAccess.length > 0) {
        extractedName = context.propertyAccess[0];
        confidence = 0.8;
   ...
   */
  private generateSemanticName(pattern: BehavioralPattern, context: SemanticContext): string {
    const baseName = pattern.extractedName || 'item';
    const cleanName = this.cleanPropertyName(baseName);
    
    switch (pattern.type) {
      case 'property_extractor':
        return `get${this.capitalize(cleanName)}` || `extract${this.capitalize(cleanName)}`;
      
      case 'comparator':
        return `compare${this.capitalize(cleanName)}` || `${cleanName}Comparator`;
      ...
      case 'accumulator':
        return `accumulate${this.capitalize(cleanName)}` || `sum${this.capitalize(cleanName)}`;
      
      case 'transformer':
        return `transform${this.capitalize(cleanName)}` || `process${this.capitalize(cleanName)}`;
      
      case 'mapper':
        return `map${this.capitalize(cleanName)}` || `${cleanName}Mapper`;
      
      case 'validator':
        return `validate${this.capitalize(cleanName)}` || `${cleanName}Validator`;
   ...
   */
  private extractContextualText(node: Parser.SyntaxNode, source: string): string {
   ...
   */
  private findParentCallExpression(node: Parser.SyntaxNode): Parser.SyntaxNode | null {
    let current = node.parent;
    while (current) {
      if (current.type === 'call_expression') {
        return current;
   ...
   */
  private extractMethodFromCall(callNode: Parser.SyntaxNode, source: string): string | null {
   ...
   */
  private extractPropertyName(node: Parser.SyntaxNode, source: string): string | null {
   ...
   */
  private extractOperator(node: Parser.SyntaxNode, source: string): string | null {
    for (let i = 0; i < node.childCount; i++) {
      const child = node.child(i);
   ...
   */
  private inferReturnType(node: Parser.SyntaxNode, source: string): 'boolean' | 'number' | 'string' | 'object' | 'array' | 'unknown' {
    ...
    // Number patterns
    if (/^\d+(\.\d+)?$/.test(text) || text.includes('+') || text.includes('-') || text.includes('*') || text.includes('/')) {
      return 'number';
    }
    ...
    // Array patterns
    if (text.startsWith('[') || text.includes('.map(') || text.includes('.filter(')) {
      return 'array';
    }
   ...
   */
  private analyzeParameterUsage(node: Parser.SyntaxNode, paramName: string, source: string): string[] {
    const usage: string[] = [];
   ...
   */
  private hasAccumulatorPattern(node: Parser.SyntaxNode, source: string): boolean {
    const params = this.extractParameters(node);
    ...
    return text.includes(`${first} +`) || text.includes(`${first}.push`) || text.includes(`${first}[`);
  }

  /**
   * Get confidence bonus based on method context
   */
  private getMethodContextBonus(methodName: string, behaviorType: BehavioralPattern['type']): number {
   ...
   */
  private extractCallContext(node: Parser.SyntaxNode, source: string): string | null {
    const callExpression = this.findParentCallExpression(node);
  ...
  private detectAdvancedPatterns(bodyText: string): Map<string, number> {
    ...
    patterns.set('async_pattern', /async|await|Promise/.test(bodyText) ? 0.9 : 0);
    patterns.set('error_handling', /try|catch|throw|Error/.test(bodyText) ? 0.8 : 0);
    patterns.set('data_validation', /\.length|\.test\(|\.includes\(|\.startsWith\(/.test(bodyText) ? 0.7 : 0);
    patterns.set('array_processing', /\.(map|filter|reduce|forEach|find|some|every)/.test(bodyText) ? 0.9 : 0);
    patterns.set('dom_manipulation', /document\.|\.querySelector|\.addEventListener/.test(bodyText) ? 0.8 : 0);
    patterns.set('type_conversion', /Number\(|String\(|Boolean\(|parseInt|parseFloat/.test(bodyText) ? 0.7 : 0);
    patterns.set('string_formatting', /\.toString|\.toFixed|template literal|\$\{/.test(bodyText) ? 0.6 : 0);
    
    return patterns;
  }

  private generateSemanticContext(bodyText: string, patterns: Map<string, number>): AISemanticContext {
  ...
  private generateIntentDescription(bodyText: string, patterns: Map<string, number>): string {
  ...
  private categorizeFunction(patterns: Map<string, number>): AISemanticContext['category'] {
  ...
  private assessComplexity(bodyText: string): AISemanticContext['complexity'] {
    ...
    const cyclomaticComplexity = (bodyText.match(/if|else|for|while|switch|case|\?|\&\&|\|\|/g) || []).length;
  ...
  private extractRelationships(bodyText: string): string[] {
    ...
    if (bodyText.includes('.map(')) relationships.push('used_in_array_transformation');
    if (bodyText.includes('.filter(')) relationships.push('used_in_array_filtering');
    if (bodyText.includes('.reduce(')) relationships.push('used_in_aggregation');
    if (/\.then|\.catch|async/.test(bodyText)) relationships.push('async_operation');
  ...
  private extractCondition(bodyText: string): string {
    const match = bodyText.match(/(\w+(?:\.\w+)*)\s*([<>=!]+)\s*(\w+(?:\.\w+)*|\d+|['"`][^'"`]*['"`])/);
    if (match) {
   ...
   */
  private generateDataFlow(type: BehavioralPattern['type'], context: SemanticContext): string {
    switch (type) {
      case 'filter':
