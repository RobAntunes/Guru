...
export class ExecutionTracer {
  ...
  async trace(params: TraceParams): Promise<ExecutionTrace> {
    console.error(`🏃 AI-Optimized execution tracing from ${params.entryPoint}`);
    
    const entrySymbol = params.symbolGraph.symbols.get(params.entryPoint);
    if (!entrySymbol) {
      throw new Error(`Entry point ${params.entryPoint} not found in symbol graph`);
    }
    ...
    const dataFlow = params.includeDataFlow !== false 
    ...
    console.error(`✅ Traced: ${stackFrames.length} frames, ${dataFlow.length} data flows, ${executionPaths.length} paths`);
  ...
  private async buildStackFrames(
    ...
    const frameQueue: Array<{ symbolId: string; depth: number; calledFrom?: StackFrame }> = [
      { symbolId: entryPoint, depth: 0, calledFrom: undefined }
    ];
    
    while (frameQueue.length > 0 && frames.length < 100) { // Safety limit
  ...
  private extractLocalVariables(symbol: SymbolNode): any[] {
    ...
    if (symbol.metadata.parameters) {
      variables.push(...symbol.metadata.parameters.map(param => ({
        name: param.name,
  ...
  private async analyzeDataFlow(
    ...
    for (const frame of stackFrames) {
      ...
      const useEdges = symbolGraph.edges.filter(
        edge => edge.from === frame.functionId && 
      ...
      for (const edge of useEdges) {
        ...
        if (targetSymbol) {
          dataFlowEdges.push({
            ...
            transformation: this.inferTransformation(edge, symbol, targetSymbol)
          });
        }
      }
    }
  ...
  private inferTransformation(edge: any, fromSymbol: SymbolNode, toSymbol: SymbolNode): string | undefined {
  ...
  private traversePath(
    ...
    if (currentNode.children.length > 0) {
      const childNode = allNodes.find(node => node.id === currentNode.children[0]);
      if (childNode) {
  ...
  private calculatePathProbability(pathNodes: string[], symbolGraph: SymbolGraph): number {
  ...
  analyzeExceptionPaths(symbolGraph: SymbolGraph): ExecutionPath[] {
    ...
    const errorSymbols = Array.from(symbolGraph.symbols.values()).filter(
      symbol => symbol.name.toLowerCase().includes('error') ||
