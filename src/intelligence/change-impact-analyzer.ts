...
export class ChangeImpactAnalyzer {
  ...
  async analyzeChangeImpact(
    ...
    console.log(`🔍 Analyzing impact of ${change.type} change to ${change.targetSymbol}...`);
    ...
    const directImpacts = await this.analyzeDirectImpacts(targetSymbol, symbolGraph, change);
    
    // Analyze indirect impacts (ripple effects)
    const indirectImpacts = await this.analyzeIndirectImpacts(targetSymbol, symbolGraph, change);
    
    // Calculate overall risk
    const risk = this.calculateChangeRisk(change, directImpacts, indirectImpacts);
    
    // Generate recommendations
    const recommendations = this.generateChangeRecommendations(change, directImpacts, indirectImpacts, risk);
    ...
    const analysis: ChangeImpactAnalysis = {
      ...
      metadata: {
        ...
        riskScore: risk.level === 'high' ? 0.8 : risk.level === 'medium' ? 0.5 : 0.2,
        safeToAutomate: risk.level === 'low' && directImpacts.length < 5,
        estimatedEffort: this.estimateChangeEffort(directImpacts, indirectImpacts),
        analysisConfidence: this.calculateAnalysisConfidence(targetSymbol, symbolGraph)
      }
    };
    
    console.log(`🎯 Change impact analysis complete: ${risk.level} risk`);
    console.log(`📊 Found ${directImpacts.length} direct impacts, ${indirectImpacts.length} indirect impacts`);
  ...
  private calculateChangeRisk(
    ...
    factors.push(`Impact scope: ${totalImpacts} affected symbols (${impactRisk.toFixed(2)})`);
    
    // Direct impact risk
    const directRisk = Math.min(0.3, directImpacts.length * 0.1);
    riskScore += directRisk;
    factors.push(`Direct impacts: ${directImpacts.length} (${directRisk.toFixed(2)})`);
  ...
  private generateChangeRecommendations(
    ...
    if (change.type === 'signature_change') {
      recommendations.push({
        type: 'compatibility',
        priority: 'high',
        description: 'Signature change detected - maintain backward compatibility',
  ...
  private isChangeTypeAffectingCaller(changeType: CodeChange['type']): boolean {
    return ['signature_change', 'behavior_change', 'deletion'].includes(changeType);
  }
  
  /**
   * Estimate effort required for change
   */
  private estimateChangeEffort(directImpacts: string[], indirectImpacts: string[]): 'low' | 'medium' | 'high' {
    const totalImpacts = directImpacts.length + indirectImpacts.length;
   ...
   */
  private calculateAnalysisConfidence(targetSymbol: SymbolNode, symbolGraph: SymbolGraph): number {
    ...
    const symbolConnectivity = this.calculateSymbolConnectivity(targetSymbol, symbolGraph);
   ...
   */
  private calculateSymbolConnectivity(symbol: SymbolNode, symbolGraph: SymbolGraph): number {
    const symbolId = Array.from(symbolGraph.symbols.entries())
      .find(([id, sym]) => sym === symbol)?.[0];
  ...
  async analyzeBatchChanges(
    ...
    for (const change of changes) {
      try {
        ...
        console.warn(`Failed to analyze change to ${change.targetSymbol}:`, error);
      }
    }
   ...
   */
  async findSafeRefactoringOpportunities(symbolGraph: SymbolGraph): Promise<CodeChange[]> {
    ...
    for (const [symbolId, symbol] of symbolGraph.symbols) {
      ...
      const connectivity = this.calculateSymbolConnectivity(symbol, symbolGraph);
