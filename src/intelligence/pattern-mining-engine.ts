...
export class PatternMiningEngine {
  ...
  private extractFeatureVector(id: string, node: SymbolNode): CodeFeatureVector {
    // Use only valid properties for MVP
    const paramCount = Array.isArray(node.metadata.parameters) ? node.metadata.parameters.length : 0;
  ...
  private clusterFeatures(featureMatrix: CodeFeatureVector[]): Cluster[] {
    ...
    const classMembers = featureMatrix.filter(f => f.features[6] === 1).map(f => f.symbolId);
    const functionMembers = featureMatrix.filter(f => f.features[7] === 1 && f.features[6] === 0).map(f => f.symbolId);
    const otherMembers = featureMatrix.filter(f => f.features[6] === 0 && f.features[7] === 0).map(f => f.symbolId);
  ...
  private centroid(members: string[], featureMatrix: CodeFeatureVector[]): number[] {
    if (members.length === 0) return this.featureNames.map(() => 0);
    const vectors = members.map(id => featureMatrix.find(f => f.symbolId === id)?.features || this.featureNames.map(() => 0));
    const sum = vectors.reduce((acc, vec) => acc.map((v, i) => v + (vec[i] || 0)), this.featureNames.map(() => 0));
  ...
  private detectOutliers(featureMatrix: CodeFeatureVector[]): Outlier[] {
    ...
    for (const f of featureMatrix) {
      const fanInZ = fanInStd ? (f.features[fanInIdx] - fanInMean) / fanInStd : 0;
      const fanOutZ = fanOutStd ? (f.features[fanOutIdx] - fanOutMean) / fanOutStd : 0;
