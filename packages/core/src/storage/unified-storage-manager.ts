...
export class UnifiedStorageManager {
  ...
  // Simple Pattern Weight Storage (for legacy compatibility)
  async storePatternWeight(name: string, weight: number, metadata?: any): Promise<void> {
    // Store directly in SurrealDB's cache table
    await this.surrealdb.cacheSet(`pattern:${name}`, {
      type: 'pattern_weight',
      name,
      weight,
      metadata,
  ...
  async getPatternWeights(): Promise<Array<{name: string, weight: number}>> {
    // For now, return empty array - would need to implement pattern scanning
    return [];
  }

  // Analysis Results Storage
  async storeAnalysisResult(result: any): Promise<void> {
    await this.surrealdb.cacheSet(`analysis:${result.type}:${Date.now()}`, result);
  }

  async getAnalysisResults(type: string, limit: number = 100): Promise<any[]> {
    // For now, return empty array - would need to implement scanning
    return [];
  }

  async getAllCachedFiles(): Promise<string[]> {
    // For now, return empty array - would need to implement file tracking
    return [];
  }

  async clearCache(): Promise<void> {
    // Clear all cache entries
    await this.surrealdb.cacheClear();
  }

  // Legacy compatibility methods
  async storeSymbol(symbol: any): Promise<void> {
    await this.createSymbol(symbol as SymbolNode);
  }

  async getSymbol(id: string): Promise<any | null> {
    // For now, return null - would need to implement symbol lookup by ID
    return null;
  }

  async createRelationship(sourceId: string, targetId: string, type: string): Promise<void> {
    await this.createSymbolRelationship({
      source: sourceId,
      target: targetId,
      type: type as any,
      metadata: {}
  ...
  async cacheAnalysis(filePath: string, analysis: any): Promise<void> {
    await this.surrealdb.cacheSet(`file:${filePath}`, analysis);
  }

  async getCachedAnalysis(filePath: string): Promise<any | null> {
    return await this.surrealdb.cacheGet(`file:${filePath}`);
  }

  async invalidateCache(filePath: string): Promise<void> {
    await this.surrealdb.cacheDelete(`file:${filePath}`);
  }
