...
import { FluidConfigEngine } from '../fluid-config/core/fluid-config-engine.js';
import { CognitiveContext, CognitiveMode, OptimalCognitiveConfig } from '../fluid-config/interfaces/fluid-config.interface.js';
...
export interface EnhancedAnalysisResult {
  ...
  fluidConfig?: OptimalCognitiveConfig;
  metadata: {
    timestamp: string;
...
export class GuruEnhanced extends GuruCore {
  ...
  private fluidConfigEngine?: FluidConfigEngine;
  ...
  /**
   * Initialize fluid config engine
   */
  async initializeFluidConfig(): Promise<void> {
    try {
      this.fluidConfigEngine = new FluidConfigEngine();
      this.logger.info('🌊 Fluid Config Engine initialized');
    } catch (error) {
      this.logger.error('Failed to initialize fluid config engine:', error);
    }
  }
  ...
  async analyzeCodebaseEnhanced(
    ...
    // Run fluid config analysis if requested
    if (options?.enableFluidConfig && options.taskInput) {
      try {
        // Initialize fluid config engine if not already initialized
        if (!this.fluidConfigEngine) {
          await this.initializeFluidConfig();
        }
        
        if (this.fluidConfigEngine) {
          // Update cognitive context with current analysis
          const taskAnalysis = await this.fluidConfigEngine['analyzeTaskDynamics'](
            options.taskInput,
            this.cognitiveContext
          );
          
          // Add to history
          this.cognitiveContext.history.push(taskAnalysis);
          
          // Generate optimal configuration
          const userProfile = {
            preferredModes: ['research', 'analytical', 'synthesis'] as CognitiveMode[],
            strengths: ['pattern-recognition', 'cross-domain-synthesis'],
          ...
          const fluidConfig = await this.fluidConfigEngine.adaptToTask(
            options.taskInput,
            this.cognitiveContext,
            userProfile
          );
          
          result.fluidConfig = fluidConfig;
          result.metadata.capabilities.push('fluid-config');
          result.metadata.cognitiveMode = fluidConfig.dynamicReconfiguration.cognitiveMode;
          
          // Update active configs
          this.cognitiveContext.activeConfigs = fluidConfig;
          this.cognitiveContext.currentMode = fluidConfig.dynamicReconfiguration.cognitiveMode;
          
          this.logger.info(`🧠 Fluid Config applied in ${fluidConfig.dynamicReconfiguration.cognitiveMode} mode`);
          this.logger.info(`  - Confidence: ${(fluidConfig.confidence * 100).toFixed(0)}%`);
          this.logger.info(`  - Universal synthesis: ${fluidConfig.cognitiveBoosts.universalSynthesis.enabled ? 'enabled' : 'disabled'}`);
          this.logger.info(`  - Thought exploration: ${fluidConfig.cognitiveBoosts.thoughtExploration.enabled ? 'enabled' : 'disabled'}`);
        }
      } catch (error) {
        this.logger.error('Fluid config analysis failed:', error);
        // Continue without fluid config
      }
    }
  ...
  /**
   * Apply fluid config to a specific task
   */
  async applyFluidConfig(taskInput: string): Promise<OptimalCognitiveConfig> {
    // Initialize fluid config engine if needed
    if (!this.fluidConfigEngine) {
      await this.initializeFluidConfig();
    }
    
    if (!this.fluidConfigEngine) {
      throw new Error('Failed to initialize Fluid Config Engine');
    }
    
    // Generate user profile based on current context
    const userProfile = {
      preferredModes: ['research', 'analytical', 'synthesis'] as CognitiveMode[],
      strengths: ['pattern-recognition', 'cross-domain-synthesis'],
      learningStyle: 'exploratory',
    ...
    this.logger.info(`🧠 Fluid Config applied: ${fluidConfig.dynamicReconfiguration.cognitiveMode} mode`);
  ...
  async cleanup(): Promise<void> {
    ...
    // Cleanup fluid config
    this.fluidConfigEngine = undefined;
  }
}